...
CSV_PATH = os.path.join(BASE_DIR, "inactive_users_inline.csv")

# NEW BOT TOKEN
BOT_TOKEN = "6968785450:AAFJbD5sRx9aCqtx-XjDe3_-Syd9X0vgEgU"
...
# INLINE KEYBOARD WITH TARGET BUTTON & URL
INLINE_KEYBOARD = {
    "inline_keyboard": [
        [
            {
                "text": "همین حالا سفارش خودت و ثبت کن",
                "url": "https://t.me/TabadolArz_Robot?start=1"
            }
        ]
    ]
}
...
def format_user_message():
    """Build warm, professional Persian marketing message with Inline button callout."""
    return (
        ...
        "⁉️ جهت ثبت سفارش و شروع معامله، روی دکمه زیر کلیک کنید 👇"
    )

def send_telegram_message(user_id, text):
    """
    Send photo + caption message via Telegram Bot API with inline keyboard button and FloodWait retry logic.
    """
    payload = {
        "chat_id": user_id,
...
def notify_admins_start(total_users, start_time):
    ...
    text = (
        f"🚀 <b>گزارش شروع پخش همگانی (ربات ثانویه - دکمه شیشه‌ای)</b>\n\n"
        ...
        f"🎯 <b>عملیات:</b> ارسال عکس و پیام + دکمه شیشه‌ای هدایت به ربات اصلی\n\n"
...
def update_admins_progress(admin_msgs, total, current, success, fail, start_time):
    ...
    text = (
        f"🔄 <b>گزارش پیشرفت پخش همگانی (دکمه شیشه‌ای)</b>\n\n"
        f"📊 <b>پیشرفت:</b> {current}/{total} ({percent:.1f}%)\n"
        f"⚡️ <b>سرعت ارسال:</b> بافت ۳۰تایی / دقیقه‌ای\n"
        f"✅ <b>ارسال موفق:</b> {success}\n"
...
def notify_admins_final(admin_msgs, total, success, fail, start_time, end_time):
    ...
    text = (
        f"📊 <b>گزارش نهایی پخش همگانی (دکمه شیشه‌ای)</b>\n\n"
...
def action_test_send():
    console.print(f"\n[cyan]🧪 Sending test photo + inline button to user ID: [bold yellow]{TEST_USER_ID}[/bold yellow]...[/cyan]")
    test_message = format_user_message()
    
    success, detail = send_telegram_message(TEST_USER_ID, test_message)
    if success:
        console.print(Panel(
            f"[bold green]✅ Test photo message with inline button sent to {TEST_USER_ID}![/bold green]\n"
            f"📷 Photo: {PROMO_IMAGE_URL}\n"
            f"🔘 Inline Button: 'همین حالا سفارش خودت و ثبت کن'\n"
            f"🔗 URL: https://t.me/TabadolArz_Robot?start=1",
...
def action_broadcast_all():
    ...
    console.print(Panel(
        f"[bold yellow]⚠️ BROADCAST WARNING (INLINE BUTTON BOT)[/bold yellow]\n\n"
        f"Bot Token: [cyan]{BOT_TOKEN[:15]}...[/cyan]\n"
        f"Target Users: [bold green]{total}[/bold green] inactive users.\n"
        ...
        f"🔘 Inline Button URL: https://t.me/TabadolArz_Robot?start=1\n"
    ...
    with Progress(
        ...
        task = progress.add_task("[cyan]Broadcasting inline messages (30/min)...", total=total)
...
def main_menu():
    while True:
        console.clear()
        console.print(Panel(
            "[bold white]درگاه امن معاملات ارزی — INLINE BUTTON NOTIFIER[/bold white]\n"
            "[dim]Marketing broadcast with Inline Button redirection[/dim]",
            subtitle="[cyan]v2.0.0 (Bot: 6968785450)[/cyan]",
        ...
        table.add_row("2", f"🧪 Send Test Photo + Inline Button to ID: [bold yellow]{TEST_USER_ID}[/bold yellow]")
        table.add_row("3", f"🚀 Start Broadcast Photo + Inline Button for ALL ({len(users)} users @ 30 msgs/min)")
